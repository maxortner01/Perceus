...
namespace pcs
{
    ...
    RenderStatus Engine::renderScene(Scene* scene) const
    {
        ...
        // Frame calculations
        {
            ...
            verticesRendered += rendAPI()->getVertexCount();
            ...
            if (totalElapsed >= seconds)
            {
                ...
                PS_CORE_INFO("RAPI: {0}  |  Avg. draw per frame: {2} |  Avg. Vrts: {3}  |  Avg. Objs: {4}  |  Avg. Process Time: {5} ms  |  Avg. Threads: {6}", 
                    settings.getAPIName(settings.api), rendAPI()->getRenderCalls() - drawCalls, (float)(rendAPI()->getRenderCalls() - drawCalls) / (float)frame,
                    commaFormat((float)verticesRendered / (float)frame), commaFormat((float)objectsRendered / (float)frame), (float)processElapsed / (float)frame,
